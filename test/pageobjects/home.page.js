...
var HomePage = Object.create(Page, {
  ...
  goToReseller: {
    value: function(resellerName) {
      ...
      if (resellerName === "Qcue") {
        ...
        browser
          .element("a*=Arizona Diamondbacks")
          .waitForVisible(this.defaultWait);
  ...
  recursiveSkipOtherCartEvent: {
    value: function() {
      if (HomePage.listingInOtherCart_Style.isExisting()) {
        browser.logger.info(
          "Skipping event with listings changes in another cart..."
        );
        //Go to next event
        HomePage.nextEvent_Button.waitForVisible(HomePage.defaultWait);
        HomePage.nextEvent_Button.click();
  ...
  recursiveSkipUselessEvents: {
    value: function() {
      if (
        HomePage.listingInOtherCart_Style.isExisting() ||
        !HomePage.listingHasReseller_Row.isExisting()
      ) {
        this.recursiveSkipEmptyEvent();
        this.recursiveSkipOtherCartEvent();
  ...
  getReviewListingsInfo: {
    value: function() {
      for (
        var int = 0;
        int < this.newPricesReviewPane_Span.value.length;
        int = int + 2
      ) {
        browser.logger.info(
          "New Listing Price Text: " +
            this.newPricesReviewPane_Span.value[int].getText()
        );
      }
    }
  },
